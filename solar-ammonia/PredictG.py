"""
This code is adopted and modified from Christopher Bartel's code on github:
https://github.com/CJBartel/predict-gibbs-energies.
The method was published on:
https://www.nature.com/articles/s41467-018-06682-4.
"""
import numpy as np
import json
import re
...
class PredictGibbsEnergy:
    ...
    def standardize_formula(self):
        ...
        if '(' not in initial_formula:
            ...
            el_num_pairs = [pair + '1' if bool(re.search(re.compile('\d'), pair)) == False else pair for pair in
                            el_num_pairs]
    ...
    def dG(self, T, vol_per_atom=False):
        ...
        if len(self.atom_names) == 1:
            ...
            return ((self.H + self.Gd_sisso(T, vol_per_atom)) * 96.485 * self.num_atoms - 96.485 * self.summed_Gi(
                T)) / self.num_atoms / 96.485
