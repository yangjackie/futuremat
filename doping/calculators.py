...
def dfpt_phonon_polarisation():
    ...
    pol_set = {
        ...
        "use_gw": True,
        "ADDGRID": True,
    }  # $, 'MAGMOM': '4*0 7*4 1*4 4*0 23*0'} #,'NELECT':270}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="workflow control for double perovskite ",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--opt", action="store_true", help="perform initial structural optimization"
    )
    parser.add_argument(
        "--pol", action="store_true", help="perform polarisation calculation"
    )
