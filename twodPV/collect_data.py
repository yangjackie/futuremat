...
from twodPV.analysis.electronic_permitivity import (
    get_geometry_corrected_electronic_polarizability,
)
...
def element_energy(db):
    print(
        "========== Collecting reference energies for constituting elements ==========="
    )
    cwd = os.getcwd()
    element_directory = cwd + "/elements/"
    for dir in [
        o
        for o in os.listdir(element_directory)
        if os.path.isdir(os.path.join(element_directory, o))
    ]:
        kvp = {}
        data = {}
        ...
        if calculator.completed:
            atoms = [i for i in read_vasp_xml(index=-1)][
                -1
            ]  # just to be explicit that we want the very last one
            e = list(_atom_dict(atoms).keys())[-1]
            reference_atomic_energies[e] = (
                atoms.get_calculator().get_potential_energy() / _atom_dict(atoms)[e]
            )
            kvp["uid"] = uid
...
def pm3m_formation_energy(db):
    print(
        "========== Collecting formation energies for bulk perovskites in Pm3m symmetry ==========="
    )
    cwd = os.getcwd()
            ...
            for b in B_site_list[i]:
                for c in C_site_list[i]:
                    ...
                    if calculator.completed:
                        ...
                        kvp["uid"] = uid
                        kvp["total_energy"] = (
                            atoms.get_calculator().get_potential_energy()
                        )
...
def __pm3m_phonon_frequencies(db):
    print(
        "========== Collecting phonon frequencies for bulk perovskites in Pm3m symmetry ==========="
    )
                ...
                for c in C_site_list[i]:
                    ...
                    if property is "phonon":
                        ...
                        try:
                            ...
                            freqs = (
                                reader.get_vibrational_eigenfrequencies_from_outcar()
                            )
                            print(freqs)
...
def randomised_structure_formation_energy(db):
    print(
        "========== Collecting formation energies for distorted perovskites  ==========="
    )
    cwd = os.getcwd()
    ...
    for i in range(len(A_site_list)):
        for a in A_site_list[i]:
            for b in B_site_list[i]:
                for c in C_site_list[i]:
                    system_name = a + b + c
                    all_rand_for_this = glob.glob(
                        base_dir + "/" + system_name + "*rand*"
                    )
...
def two_d_formation_energies(
    db, orientation="100", termination="AO2", thicknesess=[3, 5, 7, 9], large_cell=False
):
    cwd = os.getcwd()

    if not large_cell:
        base_dir = (
            cwd + "/slab_" + str(orientation) + "_" + str(termination) + "_small/"
        )
    else:
        base_dir = (
            cwd + "/slab_" + str(orientation) + "_" + str(termination) + "_full_B_octa/"
        )
...
def __two_d_100_AO_energies_large_cell(db):
    two_d_formation_energies(
        db, orientation="100", termination="AO", large_cell=True, thicknesess=[3]
    )


def __two_d_100_BO2_energies_large_cell(db):
    two_d_formation_energies(
        db, orientation="100", termination="BO2", large_cell=True, thicknesess=[3]
    )
                                ...
                                for kk in range(len(data["band_structure"][k])):
                                    for kkk in data["band_structure"][k][kk].keys():
                                        logger.info(
                                            ...
                                            + str(kkk)
                                            + " "
                                            + str(data["band_structure"][k][kk][kkk])
                                        )
                            else:
                                logger.info(
                                    str(k) + " " + str(data["band_structure"][k])
                                )
...
def get_dos_related_properties(kvp):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "Loading results from spin-polarised charge density runs unsuccessful "
            )
            try:
    ...
    if dos_run is not None:
        try:
            ...
            logger.info(
                "Spin up at Ef "
                + str(kvp["spin_up_dos_at_ef"])
                + " Spin down at Ef "
                + str(kvp["spin_down_dos_at_ef"])
            )
        except:
            pass
    try:
...
def get_out_of_plane_charge_polarisations(kvp):
    ...
    from core.data.element import atomic_numbers
    ...
    if not efolder_loaded:
        try:
            ...
            try:
                ...
                logger.warning(
                    "Error in parsing CHGCAR from spin-non-polarized calculation, skip"
                )
                try:
    ...
    try:
        ...
        a = cVector3D(
            crystal.lattice.lattice_vectors[0][0],
            crystal.lattice.lattice_vectors[0][1],
            crystal.lattice.lattice_vectors[0][2],
        )
        b = cVector3D(
            crystal.lattice.lattice_vectors[1][0],
            crystal.lattice.lattice_vectors[1][1],
        ...
        for z_value in range(NGZ):
            ...
            electron_densities -= (
                z_plane_total * (0 + resolution_z * z_value) * resolution_z
            )
...
def get_zval_from_potcar(label, use_GW=True):
    ...
    if not use_GW:
        ...
        if os.path.exists(
            settings.vasp_pp_directory + "/" + pbe_pp_choices[label] + "_GW/"
        ):
            potcar = (
                settings.vasp_pp_directory + "/" + pbe_pp_choices[label] + "_GW/POTCAR"
            )
        else:
            potcar = (
                settings.vasp_pp_directory + "/" + pbe_pp_choices[label] + "/POTCAR"
            )
...
def get_band_structures_properties(data):
    ...
    try:
        ...
        __xtal = VaspReader(input_location="./POSCAR").read_POSCAR()
        VaspWriter().write_potcar(__xtal)
        VaspWriter().write_KPOINTS(
            crystal=__xtal, filename="KPOINTS", kpoint_str=KPOINTS_string_dict[orient]
        )
        ...
        try:
            ...
            VaspWriter().write_KPOINTS(
                crystal=__xtal,
                filename="KPOINTS",
                kpoint_str=KPOINTS_string_dict[orient],
            )
            vr = BSVasprun("./vasprun_spin_BAND_temp.xml")
        except:
            logger.info(
                "No spin polarised band structure calculation results found! RETURN"
            )
            for file in [
                "./vasprun_spin_BAND_temp.xml",
                "./POTCAR",
                "./KPOINTS",
                "./KPOINTS_temp",
            ]:
    ...
    try:
        ...
        logger.warning(
            "Failed to retrieve the band structure from the vasprun.xml, RETURN"
        )
        return data
    if not bs.is_spin_polarized:
        logger.warning(
            "Not a spin polarised band structure. Don't want this now! RETURN"
        )
        return data
    ...
    if bs.is_spin_polarized:
        data["band_structure"]["direct_band_gap_energy"] = min(
            (spin_data["value"] for spin_data in direct_data.values())
        )  # band gap defined as the smaller value between the spin-up and spin-down gaps
    ...
    for spin, v in bs.bands.items():
        ...
        data["band_structure"][
            spin_label[str(spin)] + "_vbm_kpoint"
        ] = kpoint_vbm.frac_coords
    ...
    for spin, bands in list_index_band.items():
        hole_extrema.extend(
            [
                (spin, band, kpoint)
                for band in bands
                for kpoint in list_index_kpoints[spin]
            ]
        )
    hole_data = []
    ...
    for d in hole_data:
        ...
        data["band_structure"]["hole_eff_mass"].append(
            {
                "spin": str(d["spin"]),
                "eff_mass": eff_mass,
    ...
    for spin, v in bs.bands.items():
        ...
        data["band_structure"][
            spin_label[str(spin)] + "_cbm_kpoint"
        ] = kpoint_cbm.frac_coords
    ...
    for d in electron_data:
        eff_mass = fit_effective_mass(d["distances"], d["energies"], parabolic=True)
        data["band_structure"]["electron_eff_mass"].append(
            {
                "spin": str(d["spin"]),
                "eff_mass": eff_mass,
                "kpt_start": kpt_str.format(k=d["start_kpoint"].frac_coords),
...
def collect(db):
    errors = []
    steps = [
        __two_d_100_AO_electronic_structures,
