...
if apply_2D_correction:
    all_z_positions = np.array(
        [a.scaled_position.z for a in crystal.asymmetric_unit[0].atoms]
    )
...
mass_list = [
    math.sqrt(1.0 / atomic_mass_dict[a.label.upper()])
    for a in crystal.all_atoms(sort=False)
]  # Here mass is given in the atomic mass unit (a.m.u.)
    ...
    for alpha in [0, 1, 2]:  # loop around the x,y,z direction
        ...
        for i in range(len(mass_list)):  # loop around the number of atoms in the system
            for gamma in [0, 1, 2]:  # loop around the x,y,z direction
                ...
                Z_m_alpha += (
                    born_charges[i][alpha][gamma]
                    * mass_list[i]
                    * spectrum.modes[m].eigenvec[i][gamma]
                )
...
for m in range(len(spectrum.modes)):
    ...
    if np.imag(spectrum.modes[m].freq) == 0:
        e_xx = (
            (Z_tensor[m][0] * ELECTRON_CHARGE) ** 2
            / ((spectrum.modes[m].freq) ** 2 * volume)
            / EPSILON_0
            / AMU
        )
        e_yy = (
            (Z_tensor[m][1] * ELECTRON_CHARGE) ** 2
            / ((spectrum.modes[m].freq) ** 2 * volume)
            / EPSILON_0
            / AMU
        )
        ...
        if apply_2D_correction:
            av = (crystal.lattice.c / slab_thick) * (
                av_raw + (1 / N) * (1 - slab_thick / crystal.lattice.c)
            )
            sum_e_xy += av
            print(
                "mode",
                ...
                m,
                "\t",
                "{:.5f}".format(spectrum.modes[m].freq / TETRA / (2 * math.pi)),
                "\t",
                "THz",
                "\t",
                "\t",
