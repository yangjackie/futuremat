...
from core.data.element import ionic_radii
...
def plot_thickness_dependent_formation_energies(db, orientation="100", output=None):
    ...
    (rows)
    """
    figs, ((ax11, ax12, ax13, ax14), (ax21, ax22, ax23, ax24)) = plt.subplots(
        ncols=4, nrows=2, figsize=(20, 12)
    )
    thicknesses = [3, 5, 7, 9]
        ...
        for a in A_site_list[i]:
            for b in B_site_list[i]:
                for c in C_site_list[i]:
                    ...
                    two_d_en_diff = [
                        [] for _ in range(len(termination_types[orientation]))
                    ]
                    for term_type_id, term_type in enumerate(
                        termination_types[orientation]
                    ):
                        ...
                        for t in thicknesses:
                            ...
                            try:
                                row = db.get(selection=[("uid", "=", uid)])
                                twod_formation_e = row.key_value_pairs[
                                    "formation_energy"
                                ]
                                two_d_en_diff[term_type_id].append(
                                    twod_formation_e - pm3m_formation_e
                                )
                            except KeyError:
                                print("WARNING: No value for " + uid + " Skip!")
                                continue

                        axs[term_type_id].plot(
                            thicknesses,
                            two_d_en_diff[term_type_id],
                            "o:",
                            c=color,
                            alpha=0.9,
                        )
...
def plot_thickness_dependent_lowest_imaginary_frequency(
    db, orientation="100", output=None
):
    """
    Plot the thickness dependent formation energies of two-dimensional perovskite slabs in a specific orientation.
    Sructures will be categorized according to the chemistries of A/B/C sites (columns) and surface terminations
...
def plot_energy_versus_bulk_summary(db, output=None, tolerance_factor=False):
    fig, [ax_hist, ax] = plt.subplots(
        2, 1, figsize=(8, 8), sharex=True, gridspec_kw={"height_ratios": [1, 4]}
    )
                ...
                for count_a, a in enumerate(A_site_list[i]):
                    for b in B_site_list[i]:
                        for c in C_site_list[i]:
                            ...
                            if tolerance_factor:
                                tolerance_f = (
                                    ionic_radii[a][charge_state_A_site[i]]
                                    + ionic_radii[c][charge_state_C_site[i]]
                                )
                                tolerance_f /= (
                                    ionic_radii[b][charge_state_B_site[i]]
                                    + ionic_radii[c][charge_state_C_site[i]]
                                )
                            ...
                            for k in range(10):
                                ...
                                try:
                                    row = db.get(selection=[("uid", "=", uid)])
                                    randomised_formation_e = row.key_value_pairs[
                                        "formation_energy"
                                    ]
                ...
                if term_type_id == 0:
                    if not tolerance_factor:
                        ax.scatter(
                            ...
                            edgecolors=color_dict[i],
                            facecolors=color_dict[i],
                            alpha=0.5,
                            s=60,
                        )
                    else:
    ...
    print(
        "Number of two-d perovskites with relative stabilities less than 0.2 eV/ atom: "
        + str(stable_counter)
        + "/"
        + str(total)
    )
    print(
        "In which "
        + str(unstable_bulk_counter)
        + " structures have an unstable bulk perovskites"
    )
    ...
    legend_elements = [
        Patch(
            ...
            edgecolor="none",
            label="$A^{I}B^{II}_{M}X_{3}$-(AX,X$_{2}$,AX$_{3}$)",
        ),
        Patch(
            edgecolor=color_dict[0],
            facecolor="none",
            label="$A^{I}B^{II}_{M}X_{3}$-(BX$_{2}$,ABX,B)",
        ),
        Patch(
            facecolor=color_dict[1],
            edgecolor="none",
            label="$A^{I}B^{II}_{TM}X_{3}$-(AX,X$_{2}$,AX$_{3}$)",
        ),
        Patch(
            edgecolor=color_dict[1],
            facecolor="none",
            label="$A^{I}B^{II}_{TM}X_{3}$-(BX$_{2}$,ABX,B)",
        ),
        Patch(
            facecolor=color_dict[2],
            edgecolor="none",
            label="$A^{II}B^{IV}C_{3}$-(AC,C$_{2}$,AC$_{3}$)",
        ),
        Patch(
            edgecolor=color_dict[2],
            facecolor="none",
            label="$A^{II}B^{IV}C_{3}$-(BC$_{2}$,ABC,B)",
        ),
        Patch(
            facecolor=color_dict[3],
            edgecolor="none",
            label="$A^{I}B^{X}C_{3}$-(AC,C$_{2}$,AC$_{3}$)",
        ),
        Patch(
            edgecolor=color_dict[3],
            facecolor="none",
            label="$A^{I}B^{X}C_{3}$-(BC$_{2}$,ABC,B)",
        ),
    ]
    ...
    for i in range(len(A_site_list)):
                ...
                for c in C_site_list[i]:
                    ...
                    for term_type_id, term_type in enumerate(
                        ...
                        try:
                            ...
                            if twod_formation_e - pm3m_formation_e <= 10.0:
                                bulk_e_diff[term_type_id].append(
                                    pm3m_formation_e - lowest_relaxed
                                )
                                twod_e_diff[term_type_id].append(
                                    twod_formation_e - pm3m_formation_e
                                )
                                names[term_type_id].append(system_name + "$_{3}$")
                        except KeyError:
        ...
        for _p in range(len(axs)):
            ...
            for u, _ in enumerate(_x):
                ...
                if (y > 0.2) or (y < -0.2):
                    axs[_p].text(
                        x + 0.01, y + 0.01, names[_p][u], fontsize=14, color="#688291"
                    )
            ...
            axs[_p].annotate(
                str(termination_types[orientation][_p]) + "-termination",
                xy=(1, 0.9),
        ...
        axs[0].set_title(title)
        axs[1].set_xlabel(
            "$E_f^{2D,n=" + str(thick) + "}-E_{f}^{Pm\\bar{3}m}$ (eV/atom)"
        )
...
def plot_super_cell_dependent_formation_energies(
    db, orientation="100", thick=3, output=None
):
    ...
    for i in range(len(A_site_list)):
                ...
                for c in C_site_list[i]:
                    ...
                    for term_type_id, term_type in enumerate(
                        ...
                        try:
                            ...
                            twod_formation_e_small = row.key_value_pairs[
                                "formation_energy"
                            ]
                        except:
                            pass
                        ...
                        try:
                            ...
                            twod_formation_e_large = row.key_value_pairs[
                                "formation_energy"
                            ]
                        except:
                            pass

                        if (twod_formation_e_large is not None) and (
                            twod_formation_e_small is not None
                        ):
        ...
        ax11.set_xlabel(
            termination_types[orientation][0]
            + " (small cell) $E_{f}^{2D}-E_{f}^{Pm\\bar{2}m}$ (eV)"
        )
        ax11.set_ylabel(
            termination_types[orientation][0]
            + " (large cell) $E_{f}^{2D}-E_{f}^{Pm\\bar{2}m}$ (eV)"
        )

        ax21.plot(small_cell_e_diff[1], large_cell_e_diff[1], "o", c=color)
        ax21.plot(small_cell_e_diff[1], small_cell_e_diff[1], "k-")
        ax21.set_xlabel(
            termination_types[orientation][1]
            + " (small cell) $E_{f}^{2D}-E_{f}^{Pm\\bar{2}m}$ (eV)"
        )
        ax21.set_ylabel(
            termination_types[orientation][1]
...
def plot_thickness_dependent_imaginary_frequency_distribution_single_summary(
    db, output=None
):
    import math
    ...
    for thick in [3, 5, 7, 9]:
                        ...
                        for b in B_site_list[i]:
                            for c in C_site_list[i]:
                                ...
                                if gamma_point_freqs is not None:
                                    gamma_point_freqs = [
                                        (f**2).real for f in gamma_point_freqs
                                    ]
        ...
        hist, bin_edge = np.histogram(
            this_freq, bins=[-5 + 2 * j for j in range(103)], range=(-5, 200)
        )
...
def plot_thickness_dependent_imaginary_frequency_distribution(
    db, orientation="100", output=None, term_type="AO"
):
    plt.figure(figsize=(8, 6))
    ...
    __histogram_plotter(
        thickness_freqs,
        thickness_bins,
        title="[" + str(orientation) + "]//" + term + "-termination",
    )
    plt.tight_layout()
    plt.savefig(output)
...
def __histogram_plotter(thickness_freqs, thickness_bins, title):
    ...
    plt.bar(
        ...
        alpha=0.7,
        bottom=[
            thickness_freqs[3][l] + thickness_freqs[5][l]
            for l in range(len(thickness_freqs[3]))
        ],
        label="$n=7$",
        color=color_dict[7],
    )
    plt.bar(
        ...
        alpha=0.7,
        bottom=[
            thickness_freqs[3][l] + thickness_freqs[5][l] + thickness_freqs[7][l]
            for l in range(len(thickness_freqs[3]))
        ],
        label="$n=9$",
        color=color_dict[9],
...
def __frequency_histogram(db, orientation=None, term_type=None):
    ...
    for thick in [3, 5, 7, 9]:
        ...
        hist, bin_edge = np.histogram(
            this_freq, bins=[-5 + j * 0.1 for j in range(55)], range=(-5, 0)
        )
        thickness_freqs[thick] = list(hist)
        thickness_bins[thick] = list(bin_edge)[:-1]
        ...
        print(
            ...
            str(term_type),
            str(thick),
            len([f for f in this_freq if f >= -2]),
        )
    return thickness_freqs, thickness_bins


def __thickness_dependent_imaginary_frequency_vs_energy(
    db, orientation="100", term_type="AO"
):
    energy_thick = {
        3: [[] for _ in range(len(A_site_list))],
    ...
    for i in range(len(A_site_list)):
            ...
            for a in A_site_list[i]:
                for b in B_site_list[i]:
                    for c in C_site_list[i]:
                        ...
                        if (
                            (pm3m_formation_e is not None)
                            and (min_gamma_freq is not None)
                            and (two_d_formation_energy is not None)
                        ):
                            energy_thick[thick][i].append(two_d_formation_energy - 0)
                            freq_thick[thick][i].append(min_gamma_freq)

                            if (two_d_formation_energy <= 0.2) and (
                                min_gamma_freq >= -2
                            ):
                                ...
                                if (two_d_formation_energy <= min_energy) and (
                                    min_gamma_freq >= max_freq
                                ):
        ...
        print(
            most_frequent(stable_chemical_space[0]),
            most_frequent(stable_chemical_space[1]),
...
def __freq_energy_plotter(ax, energy_thick, freq_thick, title, legend=False):
    ...
    for i in [0, 1, 2, 3]:
        for thick in [3, 5, 7, 9]:
            ax.plot(
                freq_thick[thick][i],
                energy_thick[thick][i],
                "o",
                color=chem_color_dict[i],
                alpha=0.5,
                label="$n=$" + str(thick),
                ms=5 + 1.5 * thick,
            )
            ...
            for j in range(len(freq_thick[thick][i])):
                if (freq_thick[thick][i][j] >= -2) and (
                    energy_thick[thick][i][j] <= 0.2
                ):
                    counter += 1
            print(
                str(i) + " " + str(thick) + "  no. of stable structures " + str(counter)
            )
        # if max(energy_thick[thick][i]) > 10:
        #    set_y = True

    ax.add_patch(
        patches.Rectangle((-2, -2.5), 2.3, 2.7, alpha=0.4, facecolor="#F1BA46")
    )
    ...
    if legend:
        legend_elements = [
            ...
            Line2D(
                ...
                marker="o",
                color="#07000E",
                label="$A^{I}B_{M}^{II}X_{3}$",
                markerfacecolor="#07000E",
                markersize=5 + 1.5 * 3,
                lw=0,
            ),
            Line2D(
            ...
            Line2D(
                ...
                marker="o",
                color="#522E75",
                label="$A^{II}B^{IV}C_{3}$",
                markerfacecolor="#522E75",
                markersize=5 + 1.5 * 3,
                lw=0,
            ),
            Line2D(
                    ...
                    for a in A_site_list[i]:
                        for b in B_site_list[i]:
                            for c in C_site_list[i]:
                                ...
                                two_d_formation_energy = row.key_value_pairs[
                                    "formation_energy"
                                ]

                                uid = system_name + "3_pm3m"
                                row = db.get(selection=[("uid", "=", uid)])
                                pm3m_formation_e = row.key_value_pairs[
                                    "formation_energy"
                                ]
                                ...
                                if (
                                    ...
                                    thickness_energies[thick].append(
                                        two_d_formation_energy
                                    )  # - pm3m_formation_e)
                                    thickness_freqs[thick].append(min_gamma_freq)
        plt.plot(
            thickness_freqs[thick],
            thickness_energies[thick],
            "o",
            color=color_dict[thick],
            alpha=0.7,
            label="$n=$" + str(thick),
            ms=5,
        )
...
def plot_bulk_and_2D_ionic_dielectric_constants(
    db, output=None, stable_structure_only=True
):
    import scipy
    ...
    epsilon_SrTiO = trace(
        db.get(selection=[("uid", "=", "SrTiO3_pm3m")]).data["dielectric_ionic_tensor"]
    )
    print("SrTiO", epsilon_SrTiO)
    epsilon_BaTiO = trace(
        db.get(selection=[("uid", "=", "BaTiO3_pm3m")]).data["dielectric_ionic_tensor"]
    )
    print("BaTiO", epsilon_BaTiO)
    epsilon_BaZrO = trace(
        db.get(selection=[("uid", "=", "BaZrO3_pm3m")]).data["dielectric_ionic_tensor"]
    )
    print("BaZrO", epsilon_BaZrO)
    epsilon_CsSnF = trace(
        db.get(selection=[("uid", "=", "CsSnF3_pm3m")]).data["dielectric_ionic_tensor"]
    )
            ...
            for term_id, term in enumerate(termination_types[orient]):
                ...
                for thick in thicknesses:
                    ...
                    for a in A_site_list[i]:
                        for b in B_site_list[i]:
                            for c in C_site_list[i]:
                                ...
                                row = db.get(selection=[("uid", "=", uid)])
                                try:
                                    bulk_dielectric_tensor = row.data[
                                        "dielectric_ionic_tensor"
                                    ]
                                except:
                                    continue
                                ...
                                supercell_c = (
                                    structure_2d.get_cell_lengths_and_angles()[2]
                                )
                                ...
                                all_z_positions = np.array(
                                    [
                                        a.scaled_position.z
                                        for a in structure.asymmetric_unit[0].atoms
                                    ]
                                )
                                all_z_positions = all_z_positions - np.round(
                                    all_z_positions
                                )
                                all_z_positions = [
                                    z * supercell_c for z in all_z_positions
                                ]
                                slab_thick = max(all_z_positions) - min(all_z_positions)

                                try:
                                    twod_dielectric_tensor = row_2d.data[
                                        "dielectric_ionic_tensor"
                                    ]
                                except:
                                    continue
                                ...
                                if not skip_this:
                                    ...
                                    if non_zero(twod_dielectric_tensor):
                                        twod_epsilon_inplane = trace_inplane(
                                            twod_dielectric_tensor
                                        )
                                        twod_epsilon_inplane = (
                                            supercell_c / slab_thick
                                        ) * (twod_epsilon_inplane - 1.0) + 1.0

                                        twod_epsilon_outplane = trace_outplane(
                                            twod_dielectric_tensor
                                        )
                                        twod_epsilon_outplane = 1.0 / (
                                            (supercell_c / slab_thick)
                                            * (1.0 / twod_epsilon_outplane - 1.0)
                                            + 1.0
                                        )

                                    if (
                                        (bulk_epsilon is not None)
                                        and (twod_epsilon_inplane is not None)
                                        and (twod_epsilon_outplane is not None)
                                    ):
                                        # print(uid_2d, twod_epsilon_inplane, twod_epsilon_outplane)
                                        bulk_epsilons.append(bulk_epsilon)
                                        twod_epsilons_inplane.append(
                                            twod_epsilon_inplane
                                        )
                                        twod_epsilons_outplane.append(
                                            twod_epsilon_outplane
                                        )
                    ...
                    print(
                        ...
                        i,
                        "{:.3f}".format(
                            scipy.stats.pearsonr(bulk_epsilons, twod_epsilons_inplane)[
                                0
                            ]
                        ),
                    )
                ...
                if plot_this:
                    plt.plot(
                        [min(bulk_epsilons), max(bulk_epsilons)],
                        [min(bulk_epsilons), max(bulk_epsilons)],
                        "k--",
                        lw=1.5,
                    )
                ...
                if slot == 1:
                    textstr = "AX-termination"
                if slot == 2:
                    textstr = "ABX-termination"
                if slot == 3:
                    textstr = "AX$_{3}$-termination"
                if slot == 4:
                    textstr = "BX$_{2}$-termination"
                if slot == 5:
                    textstr = "X$_{2}$-termination"
                if slot == 6:
                    textstr = "B-termination"

                if plot_this:
                    if slot == 1:
                        plt.scatter(
                            [epsilon_BaTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaTiO3_100_AO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                            edgecolor="r",
                        )
                        plt.scatter(
                            [epsilon_SrTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("SrTiO3_100_AO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_BaZrO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaZrO3_100_AO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_CsSnF for _ in range(4)],
                            [
                                corrected_trace_inplane("CsSnF3_100_AO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                    ...
                    if slot == 2:
                        plt.scatter(
                            [epsilon_BaTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaTiO3_110_ABO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_SrTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("SrTiO3_110_ABO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_BaZrO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaZrO3_110_ABO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_CsSnF for _ in range(4)],
                            [
                                corrected_trace_inplane("CsSnF3_110_ABO_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                    ...
                    if slot == 3:
                        plt.scatter(
                            [epsilon_BaTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaTiO3_111_AO3_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_SrTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("SrTiO3_111_AO3_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_BaZrO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaZrO3_111_AO3_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_CsSnF for _ in range(4)],
                            [
                                corrected_trace_inplane("CsSnF3_111_AO3_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                    ...
                    if slot == 4:
                        plt.scatter(
                            [epsilon_BaTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaTiO3_100_BO2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_SrTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("SrTiO3_100_BO2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_BaZrO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaZrO3_100_BO2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                    ...
                    if slot == 5:
                        plt.scatter(
                            [epsilon_BaTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaTiO3_110_O2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_SrTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("SrTiO3_110_O2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_BaZrO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaZrO3_110_O2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_CsSnF for _ in range(4)],
                            [
                                corrected_trace_inplane("CsSnF3_110_O2_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                    ...
                    if slot == 6:
                        plt.scatter(
                            [epsilon_BaTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaTiO3_111_B_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_SrTiO for _ in range(4)],
                            [
                                corrected_trace_inplane("SrTiO3_111_B_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_BaZrO for _ in range(4)],
                            [
                                corrected_trace_inplane("BaZrO3_111_B_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
                        ...
                        plt.scatter(
                            [epsilon_CsSnF for _ in range(4)],
                            [
                                corrected_trace_inplane("CsSnF3_111_B_" + str(t))
                                for t in thicknesses
                            ],
                            s=[t * 7.5 for t in thicknesses],
                            facecolor="None",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Switches for analyzing the energy landscapes of 2D perovskites",
    ...
    if args.vbulk:
        if not args.summary:
            plot_energy_versus_bulk(
                args.db, orientation=args.orient, output=args.output
            )
        if args.summary:
            plot_energy_versus_bulk_summary(
                args.db, output=args.output, tolerance_factor=args.tolerance_factor
            )

    if args.size_dependent_energies:
        plot_thickness_dependent_formation_energies(
            args.db, orientation=args.orient, output=args.output
        )

    if args.size_dependent_frequencies:
        plot_thickness_dependent_lowest_imaginary_frequency(
            args.db, orientation=args.orient, output=args.output
        )

    if args.freq_distribute:
        if not args.summary:
            plot_thickness_dependent_imaginary_frequency_distribution(
                args.db,
        ...
        if args.summary:
            ...
            plot_thickness_dependent_imaginary_frequency_distribution_single_summary(
                args.db, output=args.output
            )

    if args.freq_energy_plot:
        if args.summary:
            plot_thickness_dependent_imaginary_frequency_energy_summary(
                args.db, output=args.output
            )
    ...
    if args.ionic_dielectric:
        plot_bulk_and_2D_ionic_dielectric_constants(
            args.db, output=args.output, stable_structure_only=False
        )
