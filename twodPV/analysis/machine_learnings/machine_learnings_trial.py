...
unique_element_list = list(
    itertools.chain.from_iterable(A_site_list + B_site_list + C_site_list)
)
...
def descriptor_entry_dict():
    ...
    sorted_atomic_numbers = list(
        sorted([atomic_numbers[e] for e in unique_element_list])
    )
                ...
                for a in A_site_list[i]:
                    for b in B_site_list[i]:
                        for c in C_site_list[i]:
                            ...
                            _ac_frac_dict = {
                                k: _ac_dict[k] / crystal.total_num_atoms()
                                for k in _ac_dict.keys()
                            }
                            chemical_descriptor = np.array(
                                [0.0 for _ in range(len(unique_element_list) + 2)]
                            )
                            for k in _ac_frac_dict.keys():
                                chemical_descriptor[descriptor_entry_dict()[k]] = (
                                    _ac_frac_dict[k]
                                )
...
design_matrix_embedded = TSNE(n_components=2, perplexity=10).fit_transform(
    design_matrix
)
...
ax[0, 0].scatter(
    design_matrix_embedded[:, 0],
    design_matrix_embedded[:, 1],
    marker="o",
    facecolor=composition_face_colors,
    edgecolors=composition_edge_colors,
    alpha=0.5,
    s=sizes,
...
fig.colorbar(
    ...
    label="$E_f^{2D,n=3}-E_{f}^{Pm\\bar{3}m}$ (eV/atom)",
)
ax[0, 2].axis("off")
