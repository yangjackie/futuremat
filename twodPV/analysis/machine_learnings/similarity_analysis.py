...
def map_structures_with_given_orientation(
    db,
    orientation="100",
    kernel="average",
    rcut=4,
                ...
                for c in C_site_list[i]:
                    ...
                    for t, term in enumerate(termination_types[orientation]):
                        for thick in [3, 5, 7, 9]:
                            ...
                            if row is not None:
                                try:
                                    formation_e = row.key_value_pairs[
                                        "formation_energy"
                                    ]
                                except KeyError:
                                    continue
    ...
    if descriptor == "soap":
        all_desc = [
            row_to_soap_descriptor(
                all_systems_rows[i],
                centers=center_lists[i],
                rcut=rcut,
                nmax=nmax,
                lmax=lmax,
                sigma=sigma,
        ...
        for j in range(i + 1, len(all_desc)):
            if descriptor == "soap":
                this_kernel = sum(
                    [
                        local_kernel(
                            centre_I_descs=all_desc[i][k],
                            centre_II_descs=all_desc[j][k],
                            kernel=kernel,
    ...
    kpca = KernelPCA(
        n_components=None, kernel="precomputed", fit_inverse_transform=False
    )
    ...
    if save_kernel_kpca:
        ...
        pickle.dump(
            data_dict,
            open(
                "similarity_kernel_" + str(orientation) + "_" + str(descriptor) + ".bp",
                "wb",
            ),
        )
        ...
        if color_by == "lattice_const":
            ...
            plt.scatter(
                ...
                norm=colors.SymLogNorm(
                    linthresh=0.03, linscale=0.03, vmin=-1.5, vmax=1.5, base=10
                ),
            )
    ...
    if color_by == "C":
        legend_elements = [
            Patch(facecolor=anion_color_dict[k], edgecolor="none", label=k)
            for k in anion_color_dict.keys()
        ]
    elif color_by == "B":
        legend_elements = [
            Patch(facecolor=b_color_dict[k], edgecolor="none")
            for k in b_color_dict.keys()
        ]
    elif color_by == "thickness":
        legend_elements = [
            Patch(
                facecolor=thick_color_dict[k], edgecolor="none", label="$n=$" + str(k)
            )
            for k in thick_color_dict.keys()
        ]
    elif color_by == "term":
        legend_elements = [
            Patch(
                facecolor="#FFD662FF",
                edgecolor="none",
                label=termination_types[orientation][0]
                .replace("O", "X")
                .replace("2", "$_2$")
                .replace("3", "$_3$"),
            ),
            Patch(
                facecolor="#00539CFF",
                edgecolor="none",
                label=termination_types[orientation][1]
                .replace("O", "X")
                .replace("2", "$_2$")
                .replace("3", "$_3$"),
            ),
...
def row_to_soap_descriptor(row, rcut=4, nmax=8, lmax=9, sigma=0.05, centers=["B"]):
    ...
    __atomic_numbers = list(
        set(
            [atomic_numbers[a.label] for a in crystal.all_atoms(sort=True, unique=True)]
        )
    )

    # initializing the descriptor
    desc = SOAP(
        species=__atomic_numbers,
        rcut=rcut,
        nmax=nmax,
        lmax=lmax,
        sigma=sigma,
...
def row_to_mbtr_descriptor(row):
    ...
    k1 = {
        ...
        "grid": {
            ...
            "sigma": 0.05,
            "n": 200,
        },  # take into account all elements in the periodic table
    }
...
def compare_b_site_coordination_environment_to_bulk(
    db,
    orientation="100",
    termination="AO",
    kernel="average",
            ...
            for b in B_site_list[i]:
                for c in C_site_list[i]:
                    ...
                    _atomic_numbers = list(
                        set(
                            [
                                atomic_numbers[a.label]
                                for a in _bulk_system.all_atoms(sort=True, unique=True)
                            ]
                        )
                    )
                    b_site_indicies = [
                        atom.index for atom in bulk_atoms if atom.symbol == b
                    ]

                    desc = SOAP(
                        species=_atomic_numbers,
                        rcut=rcut,
                        nmax=nmax,
                        lmax=lmax,
                        sigma=sigma,
                        ...
                        if twod_formation_e is not None:
                            ...
                            _twod_atomic_numbers = list(
                                set(
                                    [
                                        atomic_numbers[a.label]
                                        for a in twod_system.all_atoms(
                                            sort=True, unique=True
                                        )
                                    ]
                                )
                            )
                            twod_b_site_indicies = [
                                atom.index for atom in twod_atoms if atom.symbol == b
                            ]
                            desc = SOAP(
                                species=_twod_atomic_numbers,
                                rcut=rcut,
                                nmax=nmax,
                                lmax=lmax,
                                sigma=sigma,
                            ...
                            two_d_feature = desc.create(
                                twod_atoms, positions=twod_b_site_indicies
                            )
                            two_d_feature = normalize(two_d_feature)

                            similarity = local_kernel(
                                bulk_feature, two_d_feature, kernel=kernel
                            )
    ...
    for thick in [3, 5, 7, 9]:
        ...
        plt.plot(
            x_grid, pdf, "-", lw=1.5, c=color_dict[thick], label="$n=$" + str(thick)
        )
    ...
    plt.tight_layout()
    plt.savefig(
        "similarity_distr_" + str(orientation) + "_" + str(termination) + ".pdf"
    )


def local_kernel(
    centre_I_descs=None, centre_II_descs=None, kernel="average", descriptor="soap"
) -> float:
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Machine-Learning Analysis for 2D perovskites",
    ...
    parser.add_argument(
        "-map", "--map", action="store_true", help="Produce a map of the structures"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="soap",
        help="Type of descriptor to be used",
        choices=["soap", "mbtr"],
    )
    parser.add_argument(
        "-kl",
    ...
    if args.compare_to_bulk:
        compare_b_site_coordination_environment_to_bulk(
            args.db,
            orientation=args.orient,
            termination=args.terminations,
            kernel=args.kernel,
            rcut=args.rcut,
            lmax=args.lmax,
            nmax=args.nmax,
        ...
        map_structures_with_given_orientation(
            args.db,
            orientation=args.orient,
            kernel=args.kernel,
