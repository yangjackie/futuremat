...
chem_dict = {0: '$A^{I}B^{II}_{M}X_{3}$', 1: '$A^{I}B^{II}_{TM}X_{3}$', 2: '$A^{II}B^{IV}C_{3}$',
             3: '$A^{I}B^{X}C_{3}$'}
    ...
    if c_id == 1:
        ...
        for i in range(len(A_site_list)):
            legend_elements.append(
                Line2D([0], [0], marker=symbol_dict[i], markerfacecolor='#00743F', markeredgecolor='None', color='w',
                       markersize=7, label=chem_dict[i]))
