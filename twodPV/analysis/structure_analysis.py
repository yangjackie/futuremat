...
def BO_bond_vectors_in_bulk_perovskite(db, a, b, c):
    ...
    for bonds in analyzer.get_bonds(b, c)[0]:
        v = cVector3D(
            *atoms.get_distances(bonds[0], [bonds[1]], mic=True, vector=True)[0]
        )
        v = cVector3D(*v)
...
def BO_bond_vectors_in_twod_perovskites(
    db, a, b, c, orientation, termination, thickness
):
    system_name = a + b + c
    uid = (
        system_name
        ...
        for term_id, term_type in enumerate(termination_types[orientation]):
                ...
                for a in A_site_list[i]:
                    for b in B_site_list[i]:
                        for c in C_site_list[i]:
                            bo_vecs_in_pm3m = BO_bond_vectors_in_bulk_perovskite(
                                db, a, b, c
                            )
                            bulk_out_comps = [
                                abs(bv.dot(_or_vec)) for bv in bo_vecs_in_pm3m
                            ]

                            bulk_inplane_comps = [
                                bv
                                - _or_vec.vec_scale(bv.dot(_or_vec) / _or_vec.l2_norm())
                                for bv in bo_vecs_in_pm3m
                            ]
                            bulk_inplane_comps = [
                                v.l2_norm() for v in bulk_inplane_comps
                            ]
                            ...
                            _delta_inplane_bulk = [
                                v / bulk_in_plane_max for v in bulk_inplane_comps
                            ]
                            _delta_inplane_bulk = (
                                -1.0
                                * sum(_delta_inplane_bulk)
                                / len(_delta_inplane_bulk)
                            )

                            _delta_out_of_plane_bulk = [
                                v / bulk_out_comp_max for v in bulk_out_comps
                            ]
                            _delta_out_of_plane_bulk = (
                                -1.0
                                * sum(_delta_out_of_plane_bulk)
                                / len(_delta_out_of_plane_bulk)
                            )
                            ...
                            for thick in [3, 5, 7, 9]:
                                ...
                                print(counter)
                                two_d_BO_bonds_vec = (
                                    BO_bond_vectors_in_twod_perovskites(
                                        db, a, b, c, orientation, term_type, thick
                                    )
                                )
                                _pol = polarisation(
                                    db, a, b, c, orientation, term_type, thick
                                ...
                                two_d_out_of_plane_components = [
                                    abs(v.dot(two_d_out_of_plane_vec))
                                    / bulk_out_comp_max
                                    for v in two_d_BO_bonds_vec
                                ]
                                two_d_in_plane_components = [
                                    v
                                    - two_d_out_of_plane_vec.vec_scale(
                                        v.dot(two_d_out_of_plane_vec)
                                    )
                                    for v in two_d_BO_bonds_vec
                                ]
                                two_d_in_plane_components = [
                                    v.l2_norm() / bulk_in_plane_max
                                    for v in two_d_in_plane_components
                                ]

                                _delta_out_of_plane_two_d = sum(
                                    two_d_out_of_plane_components
                                ) / len(two_d_out_of_plane_components)
                                _delta_inplane_two_d = sum(
                                    two_d_in_plane_components
                                ) / len(two_d_in_plane_components)

                                _S_out = (
                                    _delta_out_of_plane_two_d + _delta_out_of_plane_bulk
                                )
            ...
            if term_id == 0:
                plt.scatter(
                    ...
                    alpha=0.2,
                    edgecolor="None",
                    facecolor=color_dict[orientation],
                    s=sizes,
                )
                plt.scatter(
                    sto_s,
            ...
            if slot == 1:
                legend_elements = [
                    Patch(facecolor=color_dict["100"], edgecolor="k", label="$[100]$"),
            ...
            for b in B_site_list[i]:
                for c in C_site_list[i]:
                    ...
                    for orientation in ["100", "110", "111"]:
                        ...
                        bulk_inplane_comps = [
                            bv - _or_vec.vec_scale(bv.dot(_or_vec) / _or_vec.l2_norm())
                            for bv in bo_vecs_in_pm3m
                        ]
                        ...
                        _delta_inplane_bulk = (
                            -1.0 * sum(_delta_inplane_bulk) / len(_delta_inplane_bulk)
                        )
                        ...
                        for term_type in termination_types[orientation]:
                            for thick in [9, 5, 7, 3]:
                                ...
                                out_of_plane_statistics[orientation][thick].append(
                                    _S_out
                                )
                                ...
                                colors.append(color_dict[orientation])
                                sizes.append(
                                    pylab.rcParams["lines.markersize"] ** 2 * thick / 3
                                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Switches for analyzing the structures of 2D perovskites",
