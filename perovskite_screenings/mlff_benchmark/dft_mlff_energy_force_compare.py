...
def run_single_point_from_mlff_trajectory(part=0):
    ...
    for i in range(0,len(benchmark_frames)):
        ...
        single_pt_set = {'ISPIN': 1, 'PREC': "ACCURATE", 'ALGO': 'NORMAL', 'NPAR': 28, 'ENCUT': 550, 'SIGMA':0.1, 'ISYM':-1, 'EDIFF':1E-5,
                         'LCHARG': False, 'LWAVE': False, 'use_gw': True, 'Gamma_centered': True, 'MP_points': [5, 5, 5],
        ...
        dft_force_mag = [np.linalg.norm(np.array(j)) for j in dft_vasprun.get_trajectory()[-1].site_properties['forces']]
