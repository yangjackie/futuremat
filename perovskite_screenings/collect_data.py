...
chalco_C=['O','S','Se']
chalco_A=['Ba','Mg','Ca','Sr','Be','Ra','Ti','V','Cr','Mn','Fe','Co','Ni','Pd','Pt','Cu','Ag','Zn','Cd','Hg','Ge','Sn','Pb']
...
#                        pass
                    ...
                    if phonon_converged:
                        ...
                        try:
                            ...
                            for _pp, p in enumerate(path[0]):
                                logger.info(system_name+' '+labels[_pp]+"_min_ph_freq:"+str(kvp[labels[_pp]+"_min_ph_freq"]))
                            populate_db(db, atoms, kvp, data)
                        except:
                            pass
                        if at_phonon:
                ...
                for c in C_site_list[i]:
                    ...
                    if not os.path.isfile('./kappa-m111111.hdf5'):
                        ...
                        if all_vasp_converged:
                            logger.info(system_name + '_Pm3m' + ' all VASP calculation for phono3py converged! Extracting results')
                            ...
                            for t_id,temp in enumerate(temperatures):
                                ...
                                logger.info(system_name + '_Pm3m kappa ' + ' ('+str(int(temp))+' K): '+str(kvp['kappa_'+str(int(temp))]))
                            populate_db(db, atoms, kvp, data)
