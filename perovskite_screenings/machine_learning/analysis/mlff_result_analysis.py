...
def compare_sigma_dft_mlff(trajectory=True, stepsize=1.0):
    ...
    if trajectory:
        ...
        ax.plot(time_ff_9, mlff_sigma_EDIFF_5, '-', c='#F62A00', label='$\\sigma^{(2)}$ MLFF, EDIFF=10$^{-5}$ eV', lw=5,
                alpha=0.4)
        ax.plot(time_ff_9, mlff_sigma_EDIFF_6, '-', c='#F62A00', label='$\\sigma^{(2)}$ MLFF, EDIFF=10$^{-6}$ eV', lw=4,
                alpha=0.5)
        ax.plot(time_ff_9, mlff_sigma_EDIFF_7, '-', c='#F62A00', label='$\\sigma^{(2)}$ MLFF, EDIFF=10$^{-7}$ eV', lw=3,
                alpha=0.6)
        ax.plot(time_ff_9, mlff_sigma_EDIFF_8, '-', c='#F62A00', label='$\\sigma^{(2)}$ MLFF, EDIFF=10$^{-8}$ eV', lw=2,
                alpha=0.7)
        ax.plot(time_ff_9, mlff_sigma_EDIFF_9, '-', c='#F62A00', label='$\\sigma^{(2)}$ MLFF, EDIFF=10$^{-9}$ eV', lw=1,
                alpha=0.8)
...
def statistic_box_plot():
    ...
    bp1 = plt.boxplot(box_dft_data, vert=0, showfliers=False, boxprops=boxprops, whiskerprops=boxprops,
                      capprops=boxprops)
