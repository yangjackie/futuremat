        ...
        for b_count, b in enumerate(B):
            for a_count, a in enumerate(A):
                ...
                if pm3m_formation_e is not None:
                    ...
                    all_systems.append(
                        {"name": system_name, "atoms": atoms, "indicies": indicies}
                    )
    return all_systems
                ...
                all_systems.append(
                    {
                        ...
                        "atoms": atoms,
                        "sigma": sigma,
                        "sigma_mode_averaged": sigma_mode_averaged,
                    }
                )
    return all_systems


def build_kernels_for_perovskite_systems(
    X="halides", kernel="REMatch", save_kernel=True, descriptor="SOAP"
):
    ...
    for counter, system in enumerate(all_systems):
        ...
        print(
            "Creating descriptor for system :"
            + str(counter + 1)
            + "/"
            + str(len(all_systems))
        )
            ...
            _atomic_numbers = list(
                set(
                    [
                        ...
                        for a in _system.all_atoms(sort=True, unique=True)
                    ]
                )
            )
            desc = SOAP(
...
def map_perovskites(kernel_pickle=None, color_by="sigma"):
    ...
    for i in system_info:
        ...
        if color_by == "B":
            aa = [
                _a
                for _a in a
                if _a not in ["Li", "Na", "K", "Rb", "Cs", "F", "Cl", "Br", "I"]
            ]
    ...
    if color_by == "B":
        ...
        aplot = ax.scatter(
            X_kpca[:, 0], X_kpca[:, 1], c=colors, alpha=0.7, cmap=cm, vmin=-1, vmax=5
        )
        cbaxes = ax.inset_axes([0.06, 0.12, 0.4, 0.02])
        cbar = fig.colorbar(aplot, cax=cbaxes, orientation="horizontal")
...
def main():
    ...
    parser.add_argument("--C", type=str, default="halides", help="Anion in ABCs.")
    parser.add_argument("--descriptor", type=str, default="SOAP")
    parser.add_argument("--kernel", type=str, default="REMatch")
    parser.add_argument(
        "--build_kernel",
        action="store_true",
        help="build and store the similarity kernel",
    )
    parser.add_argument(
        "--build_map",
        action="store_true",
        help="build the low-dimensional map of the landscape",
    )
    ...
    if args.build_kernel:
        build_kernels_for_perovskite_systems(
            kernel=args.kernel, descriptor=args.descriptor
        )
    if args.build_map:
