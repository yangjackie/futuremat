def calculate_high_order_phi():
    ...
    if os.path.exists('SPOSCAR'):
        reference_structure = read('SPOSCAR')
    else:
        return None
    ...
    print("referecne cell length is "+str(a))
    cs = ClusterSpace(reference_structure, [0.45*a,0.30*a])#,0.30*a])
    print("finished cluster space")
    sc=None
    #try:
    ...
    try:
        ...
        import random
        print('preparing structure container')
    ...
    if sc is not None:
       ...
       print(sigmas_2,sigmas_3,sigmas_2/sigmas_3)
       f = open('sigmas_updated.dat', 'w')
       f.write('sigma_2' + '\t' + str(sigmas_2) + '\n')
       f.write('sigma_3'+'\t'+str(sigmas_3)+'\n')
       f.write('sigma_2/sigma_3'+'\t'+str(sigmas_2/sigmas_3)+'\n')
       f.close()
       # try:
...
if __name__=="__main__":
    ...
    print('structure loaded, calculate sigma')
   ...
   #try:
    ...
    #                        force_constants='force_constants_222.hdf5')#, force_sets_filename='force_constants_222.hdf5')
