...
rc("text", usetex=True)
params = {"legend.fontsize": "11", "figure.figsize": (8, 5), "axes.labelsize": 16, "axes.titlesize": 22, "xtick.labelsize": 16, "ytick.labelsize": 16}
...
def build_radial_distribution_of_water(
    system_path="./explicit-solvent-benchmarks/H2O-SiO2-mp-554089/",
            ...
            for i in range(20):
                if random.random() > 0.6:
                    random_oxygen_sampled_distributions[i] = [
                        random_oxygen_sampled_distributions[i][j] + _this_oxygen_distributions[j] for j in range(len(_this_oxygen_distributions))
                    ]
                    random_hydrogen_sampled_distributions[i] = [
                        random_hydrogen_sampled_distributions[i][j] + _this_hydrogen_distributions[j] for j in range(len(_this_hydrogen_distributions))
                    ]
    ...
    for k in range(20):
        random_oxygen_mass_dist = [
            mass_O * random_oxygen_sampled_distributions[k][i] / random_sampled_counts[k] for i in range(len(random_oxygen_sampled_distributions[k]))
        ]
        random_hydrogen_mass_dist = [
            mass_H * random_hydrogen_sampled_distributions[k][i] / random_sampled_counts[k] for i in range(len(random_hydrogen_sampled_distributions[k]))
        ]
        total_random_mass_dist = [random_hydrogen_mass_dist[i] + random_oxygen_mass_dist[i] for i in range(len(random_oxygen_mass_dist))]
        random_density_dist[k] = [i / (base_area * delta_z * (1e-24)) for i in total_random_mass_dist]

    return total_density_dist, random_density_dist, [max_in_surface + delta_z * i for i in range(len(total_oxygen_distributions))]


def build_hydrogen_feature_vector(
    system_path="./explicit-solvent-benchmarks/H2O-SiO2-mp-554089/", subsystem="GGA-neutral", s_start=10, s_stop=50000, s_stride=20, cutoff=4.5
):
    ...
    r_cut_list = np.ones(natoms) * cutoff / 2.0
    r_cut_list[cation_index] = cutoff
    nl = NeighborList(r_cut_list, skin=0.0, sorted=False, self_interaction=False, bothways=True)
    ...
    with open("./environments/" + system_path + "-" + subsystem + "-h-dis-env.npz", "wb") as f:
...
if __name__ == "__main__":
    ...
    if args.plot_RDF:
        distribution, random_dist, distances = build_radial_distribution_of_water(system_path=args.system_path, subsystem=args.subsystem)
