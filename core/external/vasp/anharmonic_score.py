...
class AnharmonicScore(object):
    ...
    def __init__(self, ref_frame=None, md_fromes=None, potim=1, force_constants="force_constants.hdf5",
                 supercell=[1, 1, 1]):
        if isinstance(ref_frame, Crystal):
            ...
            self.ref_frame = VaspReader(input_location=args.ref_frame).read_POSCAR()
    ...
    def get_all_md_forces(self):
        ...
        self.all_forces = np.array(all_forces)
    ...
    def get_all_md_atomic_displacements(self):
        ...
        all_positions = all_positions[-len(self.all_forces):]
    ...
    def harmonic_forces(self):
        if (not hasattr(self,'_harmonic_forces')) or (hasattr(self,'_harmonic_forces') and self._harmonic_force is None):
            self._harmonic_force = np.zeros(np.shape(self.all_forces))
            for i in range(np.shape(self.all_displacements)[0]):
                self._harmonic_force[i, :, :] = -(self.force_constant @ self.all_displacements[i, :, :].flatten()).reshape(
                    self.all_displacements[0, :, :].shape)
        return self._harmonic_force

    @property
    def anharmonic_forces(self):
        if (not hasattr(self,'_anharmonic_forces')) or (hasattr(self,'_anharmonic_forces')  and self._anharmonic_forces is None):
            self._anharmonic_forces = self.all_forces - self.harmonic_forces
...
if __name__ == "__main__":
    ...
    scorer = AnharmonicScore(md_fromes=args.md_xml, ref_frame=args.ref_frame)
    scorer.structure_averaged_sigma_trajectory()
