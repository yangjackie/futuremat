...
class Vasp(Calculator):
    ...
    def _update_executable(self):
        ...
        if (self.crystal.gamma_only is True) and ("tst" not in self.executable):
            logger.info(
                "Using gamma only version VASP for calculations @ Gamma point only."
            )
    ...
    def setup(self):
        ...
        if self.write_poscar:
            if isinstance(self.crystal, Crystal):
                # then use our internal writer
                self.writer.write_structure(
                    self.crystal, filename="POSCAR", magnetic=self.magnetic
                )
            elif isinstance(self.crystal, Structure):
                # then use pymatgen structure writer
                self.crystal.to(filename="POSCAR")
        else:
            logger.info(
                "Skip writing POSCAR, use existing POSCAR (temp hack for MD runs"
            )

        self.writer.write_potcar(
            self.crystal,
            sort=False,
            unique=True,
    ...
    def _setup_kpoints(self):
        if self.kpoint_str is None:
            if not os.path.isfile("./KPOINTS"):
                logger.info(
                    "No existing KPOINTS file, autogenerate Monkhorst-Pack K-point with density of "
                    + str(self.mp_grid_density)
                    + " A^-1."
                )
                self.writer.write_KPOINTS(
                    self.crystal,
                    grid=self.mp_grid_density,
                    K_points=self.MP_points,
    ...
    def check_convergence(self, outcar=None):
        ...
        for line in outcar.readlines():
            if line.rfind("Call to ZHEGV failed") > -1:
                ...
                self.completed = False
                logger.info(
                    "VASP crashed out due to error in SCF cycles? "
                    + str(self.self_consistency_error)
                )
                break
        ...
        logger.info(
            "VASP calculation completed successfully?     " + str(self.completed)
        )
        logger.info(
            "VASP crashed out due to error in SCF cycles? "
            + str(self.self_consistency_error)
