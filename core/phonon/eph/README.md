...
```bash
python3 bandgap_renormalisation.py --optimise_structure
```
...
```bash
python3 bandgap_renormalisation.py --calculate_born_charges --structure_file CONTCAR
```
which will be carried out in the `born_charges` subdirectory. Currently, the `phonopy` binary `phonopy-vasp-born` will be used to automate the generation of the `BORN` file from `vasprun.xml` file. 
...
```bash
python3 bandgap_renormalisation.py --calculate_phonons --structure_file CONTCAR
```
