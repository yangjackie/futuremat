...
xc_defaults = {
    ...
    # vdW-DFs
    "optpbe-vdw": {"pp": "LDA", "gga": "OR", "luse_vdw": True, "aggac": 0.0},
    "optb88-vdw": {
        "pp": "LDA",
        "gga": "BO",
        "luse_vdw": True,
    ...
    "hse03": {"pp": "LDA", "gga": "PE", "lhfcalc": True, "hfscreen": 0.3},
    "hse06": {"pp": "LDA", "gga": "PE", "lhfcalc": True, "hfscreen": 0.2},
    "b3lyp": {
        "pp": "LDA",
        "gga": "B3",
        "lhfcalc": True,
    ...
    "hf": {"pp": "PBE", "lhfcalc": True, "aexx": 1.0, "aldac": 0.0, "aggac": 0.0},
}
        ...
        for line_no, line in enumerate(self.file_content):
            ...
            if start_read is not None:
                if "w=" in line:
                    ...
                    dielectric_tensor[energy] = {
                        "xx": complex(_xrow[0], _xrow[1]),
        ...
        for line in self.file_content:
            if (
                ...
                if "f/i" not in line:
                    ...
                    phonon_frequencies.append(
                        complex(0, freq)
                    )  # imaginary frequencies, instabilities!
    ...
    def read_XDATCAR(self):
        ...
        for line in self.file_content:
            ...
            if _counter in [
                ...
                3,
                4,
            ]:  # read in values of the lattice vectors from the second to forth line of the file
                _lvs.append(
                    cVector3D(
                        float(line.split()[0]),
                        float(line.split()[1]),
            ...
            if _counter == 6:
                ...
                _atomic_labels = [
                    _num_species[i] * _atomic_labels[i].split()
                    for i in range(len(_num_species))
                ]
                _atomic_labels = [
                    item for sublist in _atomic_labels for item in sublist
                ]
            ...
            if _counter > 7:
                if "Direct configuration" not in line:
                    ...
                    _atoms.append(
                        Atom(label=_atomic_labels[_atom_counter], scaled_position=coord)
                    )
                    _atom_counter += 1
                else:
                    from core.models.lattice import Lattice
            ...
            if _counter > 7:
                if _counter <= sum(_num_species) + 7:
                    ...
                    if "cartesian" in _coord_type.lower():
                        _atoms.append(
                            Atom(label=_atomic_labels[_counter - 8], position=coord)
                        )
                    elif "direct" in _coord_type.lower():
                        _atoms.append(
                            Atom(
                                label=_atomic_labels[_counter - 8],
                                scaled_position=coord,
                            )
                        )
            _counter += 1
    ...
    def read_CHGCAR(self):
        ...
        for line in self.file_content:
            if _counter > len(crystal.all_atoms()) + 7:
                ...
                elif (_counter > _chg_start) and (
                    _counter <= _chg_start + int(math.ceil(NGX * NGY * NGZ / 5))
                ):
...
class VaspWriter(object):
    ...
    def write_INCAR(
        self, filename="INCAR", default_options=default_ionic_optimisation_set, **kwargs
    ):
        ...
        for key in keylist:
            ...
            if key not in ["frame", "kwargs", "self", "SYSTEM", "filename"]:
                incar.write(
                    str(key).upper() + " = " + str(default_options[key]).upper() + "\n"
                )
        incar.close()
    ...
    def write_potcar(
        ...
        if settings.functional is not None:
            if settings.functional.lower() == "pbe":
                ...
                raise NotImplementedError(
                    "Current implementation will only concatenate PAW pseudopotential for PBE!"
                )
        else:
            raise Exception(
                "Please specify in the default configuration where to find VASP PAW pseudopotential files!"
            )
        ...
        if not use_GW:
            potcars = [
                settings.vasp_pp_directory + "/" + pbe_pp_choices[e] + "/POTCAR"
            ...
            for e in all_atom_label:
                if os.path.exists(
                    settings.vasp_pp_directory + "/" + pbe_pp_choices[e] + "_GW/"
                ):
                    potcars.append(
                        settings.vasp_pp_directory
                        + "/"
                        + pbe_pp_choices[e]
                        + "_GW/POTCAR"
                    )
    ...
    def write_structure(
        self, crystal, filename="POSCAR", direct=False, sort=True, magnetic=False
    ):
        ...
        if not magnetic:
            ...
            if sort:
                all_unqiue_labels = list(
                    sorted(set(all_atom_label), key=all_atom_label.index)
                )
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="cmd utils for VASP IO",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.025,
        help="grid spacing for generating MP Kpoints",
    )
    ...
    parser.add_argument(
        ...
        "--f",
        action="store_true",
        help="extract MD frames and write them into folder",
    )
    parser.add_argument(
        "-r",
        "--random",
    ...
    parser.add_argument(
        "-n", "--n", type=int, default=16, help="numnber of random MD frames to extract"
    )
...
def load_structure(logger=None) -> Crystal:
    """Help function to load structure for VASP optimisation from the current directory.
    The default behaviour is to restart from CONTCAR if it exists and is non-empty, allowing
    (subsequent) calculations to continue from previous optimisations. If CONTCAR does not exist,
    POSCAR is read instead."""
    ...
    elif os.path.isfile("./POSCAR") and (os.path.getsize("./POCAR") > 0):
        structure = VaspReader(input_location="./POSCAR").read_POSCAR()
        if logger is not None:
            logger.info("Start new optimisation from POSCAR")
    else:
        raise FileNotFoundError(
            "Could not find CONTCAR or POSCAR in the current directory!"
        )
    return structure
